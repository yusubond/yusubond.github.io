...
主要注意的是：如果使用`mode=host`直接在Swarm节点上发布服务的端口，并且还设置`published=<PORT>`，则会创建一个隐式限制，只能在给定的群组节点上为该服务运行一个任务。 另外，如果使用`mode=host`，并且在docker服务创建时不使用`--mode=global`标志，那么将难以知道哪些节点正在运行服务，以便将工作路由到它们。
